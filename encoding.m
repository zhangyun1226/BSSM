...
% learnRatePosList = [0.015,0.006,0.003,0.0015,0.00075];
% learnRateNegList = [0.01,0.004,0.002,0.001,0.0005];
% learnRatePosList = [0.005,0.005,0.004,0.003,0.002];
% learnRateNegList = [0.0005,0.001,0.001,0.001,0.001];
learnRatePosList = [0.0015];
learnRateNegList = [0.001];
trialNum = 1;
for ni = 1:length(learnRatePosList)
    AllEpoch = zeros(1:trialNum);
    for trial = 1:trialNum
        learnRatePos = learnRatePosList(ni);
        learnRateNeg = learnRateNegList(ni);
        columnNum = 512;
        activeNum = 10;
