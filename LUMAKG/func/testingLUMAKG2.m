...
while ~isempty(path)
    ...
    if isempty(new_path)
        targetIdx = 1;
        if length(path)>1
            targentSen = zeros(length(path),1);
            for i = 1: length(path)
                targentSen(i) = size(path{i}, 1);
            end
            targetIdx = find(targentSen == max(targentSen));
    ...
    if length(path)>5
        ...
        targetIdx = find(targentSen == max(targentSen));
        targetIdx = targetIdx(randperm(length(targetIdx),1));
        for j = 1: size(path{targetIdx}, 1)
            str = column(path{targetIdx}(j, 1)).word;
            fprintf('%s ', str);
            fprintf(frepId,'%s ',str);
